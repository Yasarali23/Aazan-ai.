// components/chat-input-v2.tsx (with agent selector)
"use client";
  ...
  return (
    <div className="w-full space-y-3">
      ...
      <div className="flex items-center gap-2 px-4 flex-wrap">
        ...
        {agentTemplate && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/30 rounded-lg hover:border-primary transition-all"
          >
            <span>{agentTemplate.icon}</span>
            <span>{agentTemplate.name}</span>
          </button>
        )}
        ...
        {!agentTemplate && (
          <button
            onClick={onSelectAgent}
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-muted border border-border rounded-lg hover:bg-muted/80 transition-colors"
          >
            <Wand2 size={16} />
      ...
      <div className="text-xs text-muted-foreground text-center pb-2">
        {isLoading
          ? `${agentTemplate?.name || "AI"} is thinking...`
          : agentTemplate
            ? `Chatting with ${agentTemplate.name}`
            : "Choose a specialist or ask any question"}
      </div>
    </div>
  );
}
